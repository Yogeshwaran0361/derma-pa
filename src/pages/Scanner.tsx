...
export const Scanner: React.FC<ScannerProps> = ({ onPredictionComplete }) => {
  ...
  const handleRunScan = async () => {
    ...
    // BLOCK SCAN IF NON-SKIN OR QUALITY CHECK FAILED!
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 sm:p-8 flex flex-col gap-6 shadow-2xl">
        ...
        {selectedFile && previewUrl ? (
          <div className="flex flex-col gap-6 items-center">
            ...
            {qualityCheck && !qualityCheck.passed ? (
              <div className={`w-full max-w-md p-6 rounded-2xl border flex flex-col gap-4 text-center shadow-xl ${
                ...
                <div className="flex flex-col items-center gap-2">
                  {qualityCheck.is_invalid_image ? (
                    <ImageOff className="w-12 h-12 text-rose-400 animate-bounce" />
                  ) : (
            ...
            {/* Run AI Scan Button (Disabled if Quality Check Failed or Non-Skin) */}
            {(!qualityCheck || qualityCheck.passed) && (
              <button
                onClick={handleRunScan}
                disabled={isAnalyzing || isCheckingQuality}
                className="w-full max-w-md py-3.5 rounded-2xl font-black text-sm shadow-xl transition-all cursor-pointer flex items-center justify-center gap-2 bg-gradient-to-r from-sky-500 to-teal-500 hover:from-sky-400 hover:to-teal-400 text-white shadow-sky-500/25 hover:scale-[1.02] disabled:opacity-50"
              >
                {isAnalyzing ? (
